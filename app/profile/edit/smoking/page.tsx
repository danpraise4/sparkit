...
export default function Smoking() {
  ...
  useEffect(() => {
    if (profile?.smoking) {
      setSelected(profile.smoking as string)
    }
  }, [profile])
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const { error } = await updateProfile({ smoking: selected })
      if (error) throw error
      toast.success('Updated successfully!')
      setShowModal(false)
      router.back()
    } catch (error) {
      console.error('Error updating smoking:', error)
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: '#F3EDF7' }}>
              ...
              >
                ...
                <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
                  ...
                  <h2 className="text-lg font-bold text-gray-900">Do you smoke?</h2>
