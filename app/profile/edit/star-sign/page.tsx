...
export default function StarSign() {
  ...
  useEffect(() => {
    if (profile?.star_sign) {
      setSelected(profile.star_sign as string)
    }
  }, [profile])
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      const { error } = await updateProfile({ star_sign: selected })
      if (error) throw error
      toast.success('Updated successfully!')
      setShowModal(false)
      router.back()
    } catch (error) {
      console.error('Error updating star sign:', error)
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: '#F3EDF7' }}>
              ...
              >
                ...
                <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between z-10">
                  ...
                  <h2 className="text-lg font-bold text-gray-900">What&apos;s your star sign?</h2>
